...
for dyn=1:Kd{n}(5)  % Dynamics
   ...
   MR.Data{n}(:,:,:,:,dyn,ph,ech,loc,mix,ex1,ex2)=MR.Data{n}(:,:,:,:,dyn,ph,ech,loc,mix,ex1,ex2).*...
   repmat(exp(1j*permute(cos(MR.Parameter.Gridder.RadialAngles{n}(:,:,:,1,dyn,ph,ech,loc,mix,ex1,ex2)')*TMP(1,:)+...
   sin(MR.Parameter.Gridder.RadialAngles{n}(:,:,:,1,dyn,ph,ech,loc,mix,ex1,ex2)')*TMP(2,:),[2 1])),[1 1 MR.UMCParameters.AdjointReconstruction.IspaceSize{n}(3) MR.UMCParameters.AdjointReconstruction.IspaceSize{n}(4)]);
