...
%         nramps=nramps*-1;end
    ...
    pcgrid=nramps(2)*(2*pi/1)*(0:0.01:1)'*ones(1,101)...
        +ones(101,1)*-nramps(1)*(2*pi/1)*(0:0.01:1);
    kx=squeeze(MR.Parameter.Gridder.Kpos{1}(1,:,:,1,:,:,:,:,:,:,:,:));kx=(kx/max(abs(kx(:)))/2)+.5;
    ky=squeeze(MR.Parameter.Gridder.Kpos{1}(2,:,:,1,:,:,:,:,:,:,:,:));ky=(ky/max(abs(ky(:)))/2)+.5;
    pcmtx=interp2(x,y,pcgrid,kx,ky);
    pcmtx=repmat(pcmtx,[1 1 Kd(3) Kd(4) 1 1 1 1 1 1 1 1]);
    MR.Data{1}=MR.Data{1}.*exp(1j.*pcmtx);
