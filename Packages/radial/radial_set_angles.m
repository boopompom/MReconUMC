...
for n=1:num_data;
	...
	if MR.UMCParameters.AdjointReconstruction.Goldenangle>0
        ...
        MR.Parameter.Gridder.RadialAngles{n}=reshape(mod((0:d_angle:(dims{n}(2)*dims{n}(5)-1)*d_angle),2*pi),[1 dims{n}(2) 1 dims{n}(5) 1 1 1 1 1 1]);
        MR.Parameter.Gridder.RadialAngles{n}=repmat(MR.Parameter.Gridder.RadialAngles{n},[1 1 1 1 1 dims{n}(7) 1 1 1 1 1]);end

	% Uniform radial linear spacing 
	if MR.UMCParameters.AdjointReconstruction.Goldenangle==0;
        d_angle=pi/(dims{n}(2));
        MR.Parameter.Gridder.RadialAngles{n}=mod((0:d_angle:(dims{n}(2)-1)*d_angle),2*pi);
        MR.Parameter.Gridder.RadialAngles{n}=repmat(MR.Parameter.Gridder.RadialAngles{n},[1 1 1 dims{n}(5) 1 dims{n}(7) 1 1 1 1 1]);
    ...
    if strcmpi(MR.Parameter.Scan.UTE,'yes') && MR.UMCParameters.AdjointReconstruction.Goldenangle==0;
       d_angle=2*pi/dims{n}(2);
       MR.Parameter.Gridder.RadialAngles{n}=mod((0:d_angle:(dims{n}(2)-1)*d_angle),2*pi);
       MR.Parameter.Gridder.RadialAngles{n}=repmat(MR.Parameter.Gridder.RadialAngles{n},[1 1 1 dims{n}(5) 1 dims{n}(7) 1 1 1 1 1]);end
