...
% MR.UMCParameters.AdjointReconstruction.SpatialResolutionRatio to an
% array.
...
for n=1:numel(MR.Data);MR.Parameter.Gridder.OutputMatrixSize{n}=[makeeven(round(MR.Parameter.Encoding.XRes(n)*MR.Parameter.Encoding.KxOversampling(n))) ...
        makeeven(round(MR.Parameter.Encoding.YRes(n)*MR.Parameter.Encoding.KyOversampling(n))) round(MR.Parameter.Encoding.ZRes(n)*MR.Parameter.Encoding.KzOversampling(n)) MR.UMCParameters.AdjointReconstruction.IspaceSize{n}(4:end)];end
