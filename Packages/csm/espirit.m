...
% Preallocate output
MR.Parameter.Recon.Sensitivities{n}=zeros(Id);
...
for dyn=1:Kd(5)  % Dynamics
    ...
    if MR.UMCParameters.IterativeReconstruction.SplitDimension==3 || strcmpi(MR.Parameter.Scan.ScanMode,'2D')
        for z=1:Kd(3)
            ...
            lowres_img=bart('nufft -i -d30:30:1 -t',...
            ...
            MR.Parameter.Recon.Sensitivities{n}(:,:,z,:,dyn,ph,ech,loc,mix,ex1,ex2,avg)=bart('ecalib -t 0.02 -m1', ksp_zerop);                             
            ...
            % Calculate sense maps
            MR.Parameter.Recon.Sensitivities{n}(:,:,:,:,dyn,ph,ech,loc,mix,ex1,ex2,avg)=bart('ecalib -t 0.02 -m1', ksp_zerop); 
    end
end % Slices
end % Dynamics
end % Echos
