...
if nargin==1 % Forward
    ...
    for n=1:numel(MR.Data)
        MR.Data{n}=permute(reshape(permute(MR.Data{n},[1 3 4 6:12 2 5]),[dims{n}(1) dims{n}(3) dims{n}(4) 1 dims{n}(6:12) dims{n}(2)*dims{n}(5) 1]),...
	        [1 12 2:11 ]);
            MR.UMCParameters.AdjointReconstruction.KspaceSize{n}(2)=[dims{n}(2)*dims{n}(5)];
	...
	if ~strcmpi(MR.UMCParameters.AdjointReconstruction.NufftSoftware,'reconframe')
        for n=1:numel(MR.Data);kd=size(MR.Parameter.Gridder.Kpos{n});kd(end+1:13)=1;
	    MR.Parameter.Gridder.Weights{n}=ipermute(reshape(permute(MR.Parameter.Gridder.Weights{n}(:,:,:,:,:,:,:,:,:,:,:,:),[1 2 5 3 4 6:12]),[kd(2) kd(3)*kd(6) 1 kd(4) 1 kd(7) kd(8) kd(9:end)]),[1 2 5 3 4 6:12]);
	    MR.Parameter.Gridder.Kpos{n}=ipermute(reshape(permute(MR.Parameter.Gridder.Kpos{n}(:,:,:,:,:,:,:,:,:,:,:,:,:),[1 2 3 6 4 5 7:13]),[3 kd(2) kd(3)*kd(6) 1 kd(4) 1 kd(7) kd(8) kd(9:end)]),[1 2 3 6 4 5 7:13]);end;end
